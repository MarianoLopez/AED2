...
/*implementaciones*/
Pila *crearPilaVacia(int size){
	static Pila *pila;
	pila = malloc(sizeof(*pila));
	(*pila).tope = -1;/*equivalente a pila->tope =-1*/
	(*pila).max = size;
	pila->arreglo = malloc(size * sizeof(pila->arreglo));/*solicitar memoria para el arreglo*/
	return pila;
}
bool pilaVacia(Pila *pila){ return (pila->tope==-1);}
bool pilaLlena(Pila *pila){return (pila->tope == (pila->max-1));}
